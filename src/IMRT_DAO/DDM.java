...
public class DDM implements Serializable {
	...
	public ArrayList<Hashtable<Integer, ArrayList<Integer>>> beamlet_to_voxel_dao_ddm;//DAO: (ArrayList)Organs  
    ...
    public DDM(Organs[] o, int[] angles, String pathfile, int[] selAngles,int totalBmlts) throws IOException{
    	...
    	for(int i=0; i<o.length; i++){
	    	...
	    	Hashtable<Integer, ArrayList<Integer>> beamlet_to_voxel = new Hashtable<Integer, ArrayList<Integer>>();
    		...
    		for(int j=0; j<angles.length; j++){
    			...
    			while(line != null){
	                ...
	                beam = Integer.parseInt(auxReader[1]) + diffBeamblets -1; //sumamos el total de beamlets del angulo anterior para calzar el beam con el id en el vector de soluco
	                radiation = Double.parseDouble(auxReader[2]);             //los ids de los beams quedan contabilizados como si toda la solucn fuese un vector (considerando todos los angulos para 1 mismo organo)
	                totalVoxel++;

	                //Guardamos los valores en el mapa de indices de voxeles
	              ...
	              //Guardamos los valores en el mapa de indices de beams
	                if(beamlet_to_voxel.get(beam) == null){
	                	voxels = new ArrayList<Integer>();
	                	voxels.add(idVoxel);
	                	beamlet_to_voxel.put(beam, voxels);
	                }else{
	                	voxels = beamlet_to_voxel.get(beam);
	                	voxels.add(idVoxel);
	                }
    		...
    		beamlet_to_voxel_dao_ddm.add(beamlet_to_voxel);
    		OrganVoxels[i]=total;
    ...
    public int writeNewDDM(int numBlts, Beam[] beams , Organs org, String fileName, String processName) throws IOException{
